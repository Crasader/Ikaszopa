...
void Settings::displayLanguageMenu()
{
    ...
    auto english = Label::createWithSystemFont("Español", "", 40);
    ...
    buttonEnglish->setTag(static_cast<int>(CustomLanguage::SPANISH));
    if (GameSettingsManager::getInstance()->getCurrentLanguage() == CustomLanguage::SPANISH) {
    ...
    auto euskara = Label::createWithSystemFont("Euskara", "", 40);
    auto buttonEuskara = MenuItemLabel::create(euskara, CC_CALLBACK_1(Settings::languageChanged, this));
    buttonEuskara->setPosition(Point(origin.x + visibleSize.width/2, origin.y + visibleSize.height*0.4));
    buttonEuskara->setTag(static_cast<int>(CustomLanguage::EUSKARA));
    if (GameSettingsManager::getInstance()->getCurrentLanguage() == CustomLanguage::EUSKARA) {
        buttonEuskara->setColor(Color3B::GREEN);
    }
    else {
        buttonEuskara->setColor(Color3B::GRAY);
    }
    items.pushBack(buttonEuskara);
    _languageButtons.push_back(buttonEuskara);
...
void Settings::languageChanged(Ref *sender)
{
    ...
    auto newLanguage = static_cast<CustomLanguage>(button->getTag());
    ...
    for (MenuItemLabel *it : _languageButtons) {
        if (static_cast<CustomLanguage>(it->getTag()) == newLanguage) {
...
{
    // exit to Main or display Settings Initial Menu, only if back key pressed (KeyCode == 6)
