...
//    float winTitleMaxSize = visibleRect.size.height - positionWinTitle.y;// - ScreenSizeManager::getHeightFromPercentage(4);
//    float scale = winTitleMaxSize / winTitle->getBoundingBox().size.height;
    //    if (scale > 1.40f) {// Check max scale factor
    //        scale = 1.40f;
    //    }
//    winTitle->setScale(scale);
